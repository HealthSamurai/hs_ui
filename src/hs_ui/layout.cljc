...
(defn nest
  ...
  (let [show? (:c/show? props)]
    [:div (when show? {:class "outline outline-1 outline-separator rounded-corner-l"})
     [:div {:class ["px-x2" (if show? "py-[var(--spacing-x2)]" "pt-[var(--spacing-x2)]")]} (:slot/control props)]
     (when show?
...
(defn navbar
  [props]
  [:div (u/merge-props
         {:class "flex border-b border-separator items-center min-h-[60px] h-[60px] justify-between w-full px-x3 gap-2"}
         props)
   (:slot/left props)
   (:slot/middle props)
   (:slot/right props)])
...
(defn control
  ...
  [:div (u/merge-props {:class "pb-[12px]"} props)
   (when (:slot/label props)
     [hs-ui.text/label {:class "w-full pb-[var(--spacing-x1point5)]"} (:slot/label props)])
   (when (:c/assistive-top? props)
     [:div {:class "w-full flex justify-between pb-[var(--spacing-x1point5)]"}
   ...
   (when-not (:c/assistive-top? props)
     (when (or (:slot/assistive props)
       ...
       [:div {:class "w-full flex gap-[var(--spacing-x1point5)] justify-between pt-[var(--spacing-x1point5)]"}
        [hs-ui.text/assistive {:class (when (:data-invalid props) "text-critical-default")}
         (:slot/assistive props)]
        [:div (:slot/assistive-right props)]]))])
...
(defn dropdown [props target overlay]
