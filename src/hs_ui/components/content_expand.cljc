...
(defn component
  ...
  [:div (hs-ui.utils/merge-props {:class root-class}
                                 (dissoc props :open?))
   [:div {:class ["w-full flex items-center justify-between" (if (:open? props) "block absolute" "invisible")]}
   ...
   [:div {:class ["w-full flex items-center justify-between" (if (:open? props) "invisible" "block absolute")]}
